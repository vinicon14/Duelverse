...
import { 
  ...
  Ban, GitBranch, ArrowUp, ArrowDown, Trash2, Megaphone, Video, UploadCloud, Link as LinkIcon, Save
} from 'lucide-react';
                        ...
                        <TableBody>
                            {filteredUsers.length > 0 ? filteredUsers.map((userToList) => (
                                <TableRow key={userToList.id} className={userToList.isBanned ? 'bg-destructive/10' : ''}>
                                    <TableCell><div className="flex items-center space-x-3"><Avatar className="h-10 w-10 border"><AvatarImage src={userToList.profilePictureUrl || ''} /><AvatarFallback>{getInitials(userToList.displayName)}</AvatarFallback></Avatar><div><p className={`font-medium ${userToList.isBanned ? 'line-through' : ''}`}>{userToList.displayName}</p><p className="text-xs text-muted-foreground">@{userToList.username}</p></div></div></TableCell>
