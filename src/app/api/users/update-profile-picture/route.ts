  ...
  try {
    ...
    if (image) {
      if (image.startsWith('data:image/')) {
        ...
        newProfilePictureUrl = `/profile-pictures/${uniqueFilename}`;

        await fs.writeFile(filePath, buffer);
      } else if (image.startsWith('http') || image.startsWith('/')) {
        newProfilePictureUrl = image;
      } else {
    ...
    const updatedUser = await updateUser(userId, { profilePictureUrl: newProfilePictureUrl }); // Use userId
